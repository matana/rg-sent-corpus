...
public class SentDetectTrainer {
	...
	public static void main(String[] args) throws IOException {
		...
		ObjectStream<String> lineStream = new PlainTextByLineStream(new MarkableFileInputStreamFactory(new File(input)), "UTF-8");
		ObjectStream<SentenceSample> sampleStream = new SentenceSampleStream(lineStream);
		SentenceModel model;

		try (FileInputStream dict = new FileInputStream("training-data/abb.xml")){
			SentenceDetectorFactory factory = new SentenceDetectorFactory("rg", false, new Dictionary(dict), new char[]{'.','?', '!'}); 
